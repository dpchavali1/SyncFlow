                      ...
                      {detailedUsers
                        ...
                        .map((user) => (
                          <tr key={user.userId} className="border-t border-gray-700 hover:bg-gray-700/50">
                            <td className="px-4 py-3 text-sm font-mono text-blue-400">{user.userId}</td>
                            <td className="px-4 py-3 text-sm">{user.messagesCount.toLocaleString()}</td>
                            <td className="px-4 py-3 text-sm">{user.storageUsedMB}MB</td>
                            <td className="px-4 py-3 text-sm text-gray-300">
                              {user.lastActivity ? new Date(user.lastActivity).toLocaleString() : 'Never'}
                            </td>
                            <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs font-medium ${user.isActive ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'}`}>{user.isActive ? 'Active' : 'Inactive'}</span></td>
                            <td className="px-4 py-3 text-center">
                              <button onClick={() => handleDeleteUser(user.userId)} disabled={deletingUser === user.userId} className="px-3 py-1 bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:bg-gray-600 text-white text-xs font-medium rounded transition-colors">
